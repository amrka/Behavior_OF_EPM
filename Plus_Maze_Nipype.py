...
def deeplabcut(video):
    ...
    copy2(h5_file, '%s%s/Plus_Maze_workflow/_subject_id_%s/DeepLabCut/' %
          (experiment_dir, working_dir, subj_no))
    copy2(labeled_video, '%s%s/Plus_Maze_workflow/_subject_id_%s/DeepLabCut/' %
          (experiment_dir, working_dir, subj_no))
    copy2(pickle, '%s%s/Plus_Maze_workflow/_subject_id_%s/DeepLabCut/' %
          (experiment_dir, working_dir, subj_no))
    copy2(csv_file, '%s%s/Plus_Maze_workflow/_subject_id_%s/DeepLabCut/' %
          (experiment_dir, working_dir, subj_no))
...
# the euclidean distance formula:  dist((x, y), (a, b)) = √(x - a)² + (y - b)²
    ...
    #################################################################################################################
    # get the mean
    # 10 is half arm width, fixed for my + maze arenas
    mean = np.mean([Z.max(axis=0), Z.min(axis=0)], axis=0) + 10

    # draw a square with mean as the center point to delineate the center of the maze
    shape1 = matplotlib.patches.RegularPolygon(
        (mean[0], mean[1]), numVertices=4, radius=40, color='r', alpha=0.5)
    ...
    # I always do this as a quality control. If there is any blue points obvious
    plt.plot(x, y, 'b.', markersize=10, alpha=0.2)

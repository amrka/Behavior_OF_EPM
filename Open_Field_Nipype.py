...
templates = {
    ...
    'open_field': 'Data/{subject_id}/open_field_{subject_id}.mp4'
}
...
def deeplabcut(video):
    ...
    copy2(h5_file, '%s%s/Open_Field_workflow/_subject_id_%s/DeepLabCut/' %
          (experiment_dir, working_dir, subj_no))
    copy2(labeled_video, '%s%s/Open_Field_workflow/_subject_id_%s/DeepLabCut/' %
          (experiment_dir, working_dir, subj_no))
    copy2(pickle, '%s%s/Open_Field_workflow/_subject_id_%s/DeepLabCut/' %
          (experiment_dir, working_dir, subj_no))
    copy2(csv_file, '%s%s/Open_Field_workflow/_subject_id_%s/DeepLabCut/' %
          (experiment_dir, working_dir, subj_no))
...
def draw_trajectory(h5_file, video):
    ...
    plt.savefig('%s_Trajectory.svg' % (filename_without_ext))
    trajectory = os.path.abspath(glob.glob('*_Trajectory.svg')[0])
    return trajectory
...
def draw_density_map(h5_file, video):
    ...
    plt.pcolormesh(xi, yi, (zi.reshape(xi.shape) - zi.min())/(zi.max()),
                   cmap='jet')  # normalize zi value to get colorbar from 0-1
    plt.colorbar(ticks=[0, 0.2, 0.4, 0.6, 0.8, 1])
    plt.axis('off')
    plt.gca().invert_yaxis()

    plt.savefig('%s_Density.svg' % (filename_without_ext))
    density_map = os.path.abspath(glob.glob('*_Density.svg')[0])
    return density_map
...
def get_metrics(h5_file, video):
  ...
  # Velocity
    ...
    plt.plot((Z.max(axis=0)[0], Z.min(axis=0)[0]),
             (Z.min(axis=0)[1], Z.max(axis=0)[1]), 'r', lw=1.5)
    plt.plot((Z.max(axis=0)[0], Z.min(axis=0)[0]),
             (Z.max(axis=0)[1], Z.min(axis=0)[1]), 'r', lw=1.5)
    ...
    plt.plot(x[x > square_x[0]][x < square_x[1]][y > square_y[0]][y < square_y[2]],
             y[x > square_x[0]][x < square_x[1]][y > square_y[0]][y < square_y[2]], 'r.')

    # now get the coordinates of the boxes at each corner of the field
    rectangle1 = plt.Rectangle(
        ((Z.min(axis=0)[0] + mean[0]) / 2, (Z.min(axis=0)[1] + mean[1]) / 2), 50, 50, fc='g', angle=180, alpha=.7)

    rectangle2 = plt.Rectangle(
        ((Z.max(axis=0)[0] + mean[0]) / 2, (Z.min(axis=0)[1] + mean[1]) / 2), 50, 50, fc='m', angle=270, alpha=.7)

    rectangle3 = plt.Rectangle(
        ((Z.max(axis=0)[0] + mean[0]) / 2, (Z.max(axis=0)[1] + mean[1]) / 2), 50, 50, fc='y', angle=-360, alpha=.7)

    rectangle4 = plt.Rectangle(
        ((Z.min(axis=0)[0] + mean[0]) / 2, (Z.max(axis=0)[1] + mean[1]) / 2), 50, 50, fc='k', angle=90, alpha=.7)
...
#     plt.gca().invert_yaxis() #we do not need it anymore, since I am plotting a frame with plt.imshow
    ...
    plt.savefig('%s_Divisions.svg' % (filename_without_ext))
    divisions = os.path.abspath(glob.glob('*_Divisions.svg')[0])
    ######################################################################################################
...
Open_Field_workflow.run(plugin='SLURM', plugin_args={
                        'dont_resubmit_completed_jobs': True, 'max_jobs': 50, 'sbatch_args': '-p gpu'})
